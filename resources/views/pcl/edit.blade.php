<x-app-layout>
    <x-slot name="title">Edit Capaian Harian</x-slot>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm dark:bg-gray-900 dark:border-gray-800">
            <h1 class="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                Edit Capaian Harian Pencacahan
            </h1>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Ubah capaian realisasi usaha dan rumah tangga yang berhasil dicacah pada tanggal terpilih.</p>

            <form method="POST" action="{{ route('daily-reports.update', $dailyReport) }}" class="mt-6 space-y-5">
                @csrf
                @method('PUT')

                <!-- Tanggal Laporan -->
                <div>
                    <label for="report_date_display" class="block mb-2 text-sm font-semibold text-gray-900 dark:text-white">Tanggal Laporan (Terkunci)</label>
                    <input type="text" id="report_date_display" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-bps-500 focus:border-bps-500 block w-full p-3 dark:bg-gray-850 dark:border-gray-800 dark:text-gray-400 focus:outline-none opacity-80 cursor-not-allowed" value="{{ $dailyReport->report_date->translatedFormat('l, d F Y') }}" readonly disabled />
                    <input type="hidden" name="report_date" value="{{ $dailyReport->report_date->format('Y-m-d') }}" />
                </div>

                <!-- SubSLS Tugas -->
                <div>
                    <label for="assignment_id_display" class="block mb-2 text-sm font-semibold text-gray-900 dark:text-white">Wilayah Tugas (SubSLS - Terkunci)</label>
                    <input type="text" id="assignment_id_display" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-bps-500 focus:border-bps-500 block w-full p-3 dark:bg-gray-850 dark:border-gray-800 dark:text-gray-400 focus:outline-none opacity-80 cursor-not-allowed" value="SubSLS {{ $dailyReport->assignment->subsls->idsubsls }} - {{ $dailyReport->assignment->subsls->sls->nmsls }}" readonly disabled />
                    <input type="hidden" name="assignment_id" value="{{ $dailyReport->assignment_id }}" />
                </div>
